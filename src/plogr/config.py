...
class Config:
    ...
    def __init__(
        self,
        *,
        system_config_file: Optional[Path] = None,
    ...
    def _determine_config_file(self) -> Path:
        ...
        Prefer the system config when it exists to match the documented precedence,
        otherwise fall back to the user config.
        """
        try:
            if self.system_config_file.exists():
                return self.system_config_file
        except OSError as err:
            logger.warning("Unable to access system config %s: %s", self.system_config_file, err)
