...
# Contains a native C++ plugin (plogr.so) → package is now arch-specific.
# BuildArch:  noarch
...
%description
Plogr is a cross-platform tool to log package installations, downloaded files, and removals from various
...
# uses an underscore (plogr-<version>) instead of the hyphenated project
# name.  Tell %autosetup to cd into that directory so %%prep succeeds.
%prep
%autosetup -S git -n plogr-%{version}
...
install -D -m 0644 src/plogr/hooks/dnf/plugin.py \
  %{buildroot}%{python3_sitelib}/dnf-plugins/plogr.py
install -D -m 0644 config/plogr.conf %{buildroot}%{_sysconfdir}/dnf/plugins/plogr.conf

# Install DNF5 plugin configuration (DNF5 core)
install -D -m 0644 config/plogr.conf %{buildroot}%{_sysconfdir}/dnf5/plugins/plogr.conf

# Install Actions file into libdnf5-plugins config dir
install -D -m 0644 libdnf5-plugin/dnf5-plugin/actions.d/plogr.actions \
  %{buildroot}%{_sysconfdir}/dnf/libdnf5-plugins/actions.d/plogr.actions

# Install systemd user service
install -D -m 0644 systemd-user/plogr.service %{buildroot}%{_userunitdir}/plogr.service

# Install man page
install -D -m 0644 docs/man/plogr.1 %{buildroot}%{_mandir}/man1/plogr.1
...
try:
    importlib.import_module("plogr")
    print("Import OK")
except Exception as exc:
    print("Failed to import plogr:", exc, file=sys.stderr)
    sys.exit(1)
PY
...
%{_bindir}/plogr
%{python3_sitelib}/plogr/
%{python3_sitelib}/plogr-%{version}.dist-info/
%{python3_sitelib}/dnf-plugins/plogr.py
%{python3_sitelib}/dnf-plugins/__pycache__/plogr.cpython-*.pyc
%{_libdir}/dnf5/plugins/plogr.so
%{_datadir}/libdnf5/plugins/actions.d/plogr.actions
%{_sysconfdir}/dnf/libdnf5-plugins/actions.d/plogr.actions
%{_sysconfdir}/dnf/plugins/plogr.conf
%{_sysconfdir}/dnf5/plugins/plogr.conf
%{_userunitdir}/plogr.service
%{_mandir}/man1/plogr.1*

%post
echo "=========================================="
echo "plogr v%{version} installed successfully!"
echo "=========================================="
echo ""
echo "QUICK START:"
echo "1. Setup configuration: plogr setup"
echo "2. Check status: plogr status"
echo "3. View manual: man plogr"
echo ""
echo "DNF PLUGIN SETUP:"
echo "Both DNF4 (Python) and DNF5 (C++) plugins are installed:"
echo ""
echo "DNF4 Plugin (Python):"
echo "  User scope:   echo 'enabled = 1' >> ~/.config/dnf/plugins/plogr.conf"
echo "  System scope:  echo 'enabled = 1' >> /etc/dnf/plugins/plogr.conf"
echo ""
echo "DNF5 Plugin (C++ - enabled by default):"
echo "  User scope:   echo 'enabled = 1' >> ~/.config/dnf5/plugins/plogr.conf"
echo "  System scope:  echo 'enabled = 1' >> /etc/dnf5/plugins/plogr.conf"
echo ""
echo "DOWNLOAD MONITORING:"
echo "To enable download monitoring (user scope only):"
echo "  systemctl --user daemon-reload"
echo "  systemctl --user enable --now plogr.service"
echo ""
echo "SCOPE CONFIGURATION:"
echo "Default scope is 'user'. To change to system scope:"
echo "  sudo plogr setup --setup system"
echo ""
echo "For more information, see: man plogr"
echo "Documentation: /usr/share/doc/plogr/"
echo "=========================================="

%postun
echo "plogr uninstalled successfully."
